...
public class DriversFragment extends Fragment implements DriversContract.View {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        mListAdapter = new DriversAdapter(new ArrayList<RegistrationCard>(0), mPresenter);
    ...
    @Override
    public void showDrivers(List<RegistrationCard> cards) {
        mListAdapter.replaceData(cards);
        mDriversViewModel.setDriverListSize(cards.size());
    }
    ...
    @Override
    public void showDriverDetailsUi(String cardId) {
    ...
    private static class DriversAdapter extends BaseAdapter {
        ...
        private List<RegistrationCard> mRegistrationCards;

        private DriversContract.Presenter mUserActionsListener;

        public DriversAdapter(List<RegistrationCard> cards, DriversContract.Presenter itemListener) {
            setList(cards);
            mUserActionsListener = itemListener;
        }

        public void replaceData(List<RegistrationCard> cards) {
            setList(cards);
        }

        private void setList(List<RegistrationCard> cards) {
            mRegistrationCards = cards;
            notifyDataSetChanged();
        }

        @Override
        public int getCount() {
            return mRegistrationCards != null ? mRegistrationCards.size() : 0;
        }

        @Override
        public RegistrationCard getItem(int i) {
            return mRegistrationCards.get(i);
        }
        ...
        public View getView(int i, View view, ViewGroup viewGroup) {
            RegistrationCard registrationCard = getItem(i);
            ...
            // We might be recycling the binding for another registrationCard, so update it.
            ...
            binding.setCard(registrationCard);
